...
$(document).ready(function () {
			...
			snapshot.forEach(function (childSnapshot) {
				...
				database.ref('/invates/' + childKey).once('value').then(function (snapshot) {
					snapshot.forEach(function (childSnapshot) {
						...
						if (childKey === USER_ID) {
							createInvitesMembros(childKey, title, content, members, hora);
						} else {
	...
	function createInvitesMembros(key, title, content, members, hora) {
		$(".box-list").prepend(`
		<div class='box-post d-flex mb-3 '>
			<div class='mr-auto'>
			<h4>${title}</h4>
			...
			<img src="img/share.png" id="id_encaminhar" class="icones" alt="Encaminhar">
			</div>`);
